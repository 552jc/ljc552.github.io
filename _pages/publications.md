...
* __Generalized Telescope Conjecture__ (in progress)<br> We introduce the atomic smashing frame, which extends  the Balmer frame from compactly generated $tt$-$\infty$-categories to an arbitrary presentably symmetric monoidal $\infty$-category $\mathcal{V}$, and formulate the telescope conjecture for $\mathcal{V}$. When $\mathcal{V}$ is stable and compactly rigidly generated, its atomic smashing frame recovers the usual Balmer frame. When $\mathcal{V}$ is an $\infty$-topos, we prove that the telescope conjecture holds for $\mathcal{V}$ if and only if $\mathcal{V}$ is a smashing field. When $\mathcal{V} = \operatorname{Mod}\_{R}(\mathrm{Sp}\_{\geq 0})$ is the $\infty$-category of connective modules over a connective $\mathbb{E}\_\infty$-ring $R$, we show that the telescope conjecture holds for $\mathcal{V}$ if and only if every idempotent ideal of $\pi_{0} R$ is generated by idempotent elements. Furthermore, in the stable setting, we establish a recollement theorem for (atomic) smashing frames and show that the spatiality of the smashing frame of $\mathrm{Sp}$ reduces to that of $\mathrm{Sp}_{T(n)}$. Finally, we introduce the notion of the Serre smashing frame together with the flat telescope conjecture. 
